...
All you need to do is to create a DNS entry like `*.api.example.com` and have it resolve to the nodes running the Gateway.
...
One way to simplify the configuration of the Gateway is to copy the `api-gateway-config` folder in the cloud, having all nodes syncing from that location.
...
There are only 2 files that won't be syncronised from the cloud:
